...
const moduleMeta: Record<
  string,
  Pick<ModuleDoc, "title" | "purpose" | "access" | "notes">
> = {
  activity_categories: {
...
function extractBlock(schema: string, blockName: "Query" | "Mutation") {
  const match = schema.match(
    new RegExp(`extend\\s+type\\s+${blockName}\\s*\\{([\\s\\S]*?)\\n\\s*\\}`),
  );
  return match?.[1] ?? "";
}
...
function extractDefinitions(schema: string, keyword: string) {
  const matches = [
    ...schema.matchAll(
      new RegExp(`^\\s*${keyword}\\s+([A-Za-z_][A-Za-z0-9_]*)`, "gm"),
    ),
  ];
...
function isMounted(mainSource: string, slug: string) {
  if (slug === "health") {
    return mainSource.includes("healthTypeDefs") &&
      mainSource.includes("healthResolvers");
  }

  const pascal = slug.split("_").map((part) =>
    part.charAt(0).toUpperCase() + part.slice(1)
  ).join("");
  const aliases = [
    pascal,
  ...
  return aliases.some((alias) =>
    ...
    mainSource.includes(`${alias}Resolvers`)
  );
}
...
function formatModuleDoc(doc: ModuleDoc) {
  ...
  const sourceLine = doc.typeDefPath
    ? `Source schema: \`${doc.typeDefPath}\``
...
async function buildGraphqlModuleDocs() {
  ...
  for await (const entry of Deno.readDir(srcDir)) {
    ...
    const typeDefUrl = new URL(
      `${entry.name}/${entry.name}.typedef.ts`,
      srcDir,
    );
    try {
  ...
  docs.push({
    ...
    ...moduleMeta.uploads ?? {
      ...
      notes: [
        "Endpoint bukan GraphQL; ditangani langsung di `main.ts` sebelum request diteruskan ke Yoga.",
      ],
    },
    mounted: true,
    operations: {
  ...
  return docs.sort((a, b) =>
    getModuleOrder(a) - getModuleOrder(b) || a.slug.localeCompare(b.slug)
  );
}
...
${
    exposed.map((doc) =>
      `- [${
        String(getModuleOrder(doc)).padStart(2, "0")
      }. ${doc.title}](./modules/${getModuleFilename(doc)}) - ${doc.purpose}`
    ).join("\n")
  }
...
${
    notMounted.length
      ? notMounted.map((doc) =>
        `- [${
          String(getModuleOrder(doc)).padStart(2, "0")
        }. ${doc.title}](./modules/${getModuleFilename(doc)}) - ${doc.notes[0]}`
      ).join("\n")
      : "- Tidak ada."
  }
...
async function main() {
  ...
  for (const doc of docs) {
    await Deno.writeTextFile(
      new URL(getModuleFilename(doc), modulesDocsDir),
      formatModuleDoc(doc),
    );
  }
